...
pfn_t free_frame(void) {
    ...
    if (frame_table[victim_pfn].mapped) {
        pte_t *victim = (pte_t*)(mem + (*frame_table[victim_pfn].process).saved_ptbr * PAGE_SIZE) + frame_table[victim_pfn].vpn;
        if (( * victim).dirty) {
            swap_write(victim, mem + (*victim).pfn * PAGE_SIZE);
            stats.writebacks ++;
        }
...
pfn_t select_victim_frame() {
    ...
    if (replacement == RANDOM) {
        ...
        static pfn_t fptr = 0;
        for (pfn_t i = fptr; i < num_entries; i++) {
            if (!frame_table[i].protected) {
                if ( i == NUM_FRAMES - 1) {
                    fptr = 0;
                } else {
        ...
        // TODO: Implement the clocksweep algorithm here 
        static pfn_t cptr = 0;
        for (pfn_t i = cptr; i < num_entries; i++) {
            if (!frame_table[i].protected) {
