...
void proc_init(pcb_t *proc) {
    ...
    fte_t * proc_f = (frame_table + page_table);
...
void proc_cleanup(pcb_t *proc) {
    ...
    for (size_t i = 0; i < NUM_PAGES; i++) {
        pte_t *base_addr = (pte_t*)(mem + (*proc).saved_ptbr * PAGE_SIZE);
        pte_t *curr_entry = base_addr + i;
        if ((*curr_entry).valid) {
            (*curr_entry).valid = 0;
            frame_table[(*curr_entry).pfn].mapped = 0;
        }
        if (swap_exists(curr_entry)) swap_free(curr_entry);
    }

    fte_t *proc_frame_table = frame_table + (*proc).saved_ptbr;
    
    (*proc_frame_table).mapped = 0;
    (*proc_frame_table).protected = 0;
