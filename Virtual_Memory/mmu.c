...
void system_init(void) {
    ...
    memset(mem, 0, PAGE_SIZE);      // frame table initialization

    frame_table = (fte_t*) mem;

    (*frame_table).protected = 1;   // protected bit set --> cannot set to victim
}
 ...
 *      Ch eck what in the pte_t struct signals that the entry is mapped in memory.
 * ----------------------------------------------------------------------------------
 */
uint8_t mem_access(vaddr_t addr, char access, uint8_t data) {
    ...
    pte_t* curr = (pte_t*)(mem + PTBR * PAGE_SIZE) + vaddr_vpn(addr);
    
    if (!(*curr).valid) {
        page_fault(addr);
    }

    (*((fte_t*) & frame_table[(*curr).pfn])).referenced = 1;

    paddr_t physical_address = (paddr_t) (((*curr).pfn) * PAGE_SIZE + vaddr_offset(addr));
